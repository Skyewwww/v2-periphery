...
contract UniswapV2Router02 is IUniswapV2Router02 {
    ...
    function _swap(uint[] memory amounts, address[] memory path, uint256[] memory fees, address _to) internal virtual {
    ...
    // requires the initial amount to have already been sent to the first pair
    function _swapSupportingFeeOnTransferTokens(address[] memory path, uint256[] memory fees, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output, uint256 fee) = (path[i], path[i + 1], fees[i]);
            (address token0,) = UniswapV2Library.sortTokens(input, output);
            IUniswapV2Pair pair = IUniswapV2Pair(UniswapV2Library.pairFor(factory, input, output, fee));
            uint amountInput;
            uint amountOutput;
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, UniswapV2Library.pairFor(factory, path[0], path[1], fees[0]), amountIn
        );
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, fees, to);
        require(
        ...
        ensure(deadline)
    {
        ...
        assert(IWETH(WETH).transfer(UniswapV2Library.pairFor(factory, path[0], path[1], fees[0]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, fees, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
        ...
        ensure(deadline)
    {
        ...
        _swapSupportingFeeOnTransferTokens(path, fees, address(this));
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut, uint256 fee)
        public
        pure
        virtual
        override
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut, uint256 fee)
        public
        pure
        virtual
        override
    ...
    function getAmountsOut(uint amountIn, address[] memory path, uint256[] memory fees)
        public
        view
        virtual
        override
    ...
    function getAmountsIn(uint amountOut, address[] memory path, uint256[] memory fees)
        public
        view
        virtual
        override
