...
library UniswapV2Library {
    ...
    function pairFor(address factory, address tokenA, address tokenB, uint256 fee) internal pure returns (address pair) {
        (address token0, address token1) = sortTokens(tokenA, tokenB);
        pair = address(uint(keccak256(abi.encodePacked(
                hex'ff',
                factory,
                keccak256(abi.encodePacked(token0, token1, fee)),
                // TODO: change to init code hash
                hex'96e8ac4277198ff8b6f785478aa9a39f403cb768dd02cbee326c3e7da348845f' // init code hash
            ))));
    }

    // fetches and sorts the reserves for a pair
    function getReserves(address factory, address tokenA, address tokenB, uint256 fee) internal view returns (uint reserveA, uint reserveB) {
        (address token0,) = sortTokens(tokenA, tokenB);
        (uint reserve0, uint reserve1,) = IUniswapV2Pair(pairFor(factory, tokenA, tokenB, fee)).getReserves();
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut, uint256 fee) internal pure returns (uint amountOut) {
        ...
        uint amountInWithFee = amountIn.mul(1e18 - fee);
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut, uint256 fee) internal pure returns (uint amountIn) {
        ...
        uint denominator = reserveOut.sub(amountOut).mul(1e18 - fee);
    ...
    function getAmountsOut(address factory, uint amountIn, address[] memory path, uint256[] memory fees) internal view returns (uint[] memory amounts) {
        ...
        require(path.length == fees.length + 1, 'UniswapV2Library: INVALID_FEES_LENGTH');
        amounts = new uint[](path.length);
        amounts[0] = amountIn;
        for (uint i; i < path.length - 1; i++) {
            (uint reserveIn, uint reserveOut) = getReserves(factory, path[i], path[i + 1], fees[i]);
    ...
    function getAmountsIn(address factory, uint amountOut, address[] memory path, uint256[] memory fees) internal view returns (uint[] memory amounts) {
        ...
        for (uint i = path.length - 1; i > 0; i--) {
            (uint reserveIn, uint reserveOut) = getReserves(factory, path[i - 1], path[i], fees[i - 1]);
